...
export class InputZip extends React.Component {
    ...
    onSubmit(e){
        ...
        const weather = 'https://api.openweathermap.org/data/2.5/weather?zip=' + this.state.zip + ',us&appid=65e3bb69c4642532490a339fdd953c7c'
        axios.get(weather)
            .then((result) => {
    ...
    onGetLoc = () => {
        ...
        const weather = 'https://api.openweathermap.org/data/2.5/weather?lat=' + this.props.coords.latitude + '&lon=' + this.props.coords.longitude + '&appid=65e3bb69c4642532490a339fdd953c7c'
        axios.get(weather)
            .then((result) => {
                console.log(result.data.main)
                const temp = Math.round(((result.data.main.temp) - 273.15) * 9/5 + 32);
